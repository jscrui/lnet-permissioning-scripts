...
// Ingress contract addresses (using AccountIngress address in both networks)
...
const FUNCTION_SIGNATURES = {
  EXECUTE_ADD_ADMIN: ethers.utils.id('executeAddAdmin(address)').slice(0, 10),
  EXECUTE_REMOVE_ADMIN: ethers.utils.id('executeRemoveAdmin(address)').slice(0, 10)
};
...
// ABI for Ingress contract - used to get the Admin contract address
...
// ABI for Admin contract - handles admin permissioning and multisig
const ACCOUNT_RULES_ABI = [
  'function addAdmin(address _newAdmin) returns (bool)',
  ...
  'function getTransaction(uint256 txId) view returns (address account,bool executed,uint256 id)',
...
// (No membership concept for Admin management)
...
async function findPendingTransactionTxId(accountRules, targetAddress, action) {
  ...
  const targetSigs = action === 'add' 
    ? [FUNCTION_SIGNATURES.EXECUTE_ADD_ADMIN]
    : [FUNCTION_SIGNATURES.EXECUTE_REMOVE_ADMIN];
  
  // Search through all pending transactions
  for (const id of ids) {
    const [, executed] = await accountRules.getTransactionPayload(id);
...
async function verifySignerPermissions(accountRules, signer1Address, signer2Address) {
  ...
  const signer1Permitted = await accountRules.isAuthorized(signer1Address);
  const signer2Permitted = await accountRules.isAuthorized(signer2Address);
 ...
 */
async function checkPermissionStatus(accountRules, account, action) {
  const isPermitted = await accountRules.isAuthorized(account);
  ...
  if (action === 'add') {
    ...
    console.log(`🗑️ Revoking Admin for account: ${account}`);
  }
 ...
 */
async function submitTransaction(accountRules1, action, account, signer1Address) {
  let tx;
  
  if (action === 'add') {
    const gasLimit = await estimateGasWithFallback(
      (baseGas) => accountRules1.estimate.addAdmin(account, { gasLimit: baseGas, gasPrice: 0 }),
      GAS_LIMIT_ADD_ACCOUNT,
      GAS_BUFFER
    );
    tx = await accountRules1.functions.addAdmin(account, { gasLimit, gasPrice: 0 });
    console.log(`📤 ➕ addAdmin(address) | sent by initiator (${signer1Address})`);
  } else {
    const gasLimit = await estimateGasWithFallback(
      (baseGas) => accountRules1.estimate.removeAdmin(account, { gasLimit: baseGas, gasPrice: 0 }),
      GAS_LIMIT_REMOVE_ACCOUNT,
      GAS_BUFFER
    );
    tx = await accountRules1.functions.removeAdmin(account, { gasLimit, gasPrice: 0 });
    console.log(`📤 ➖ removeAdmin(address) | sent by initiator (${signer1Address})`);
  }
...
async function verifyFinalState(accountRules, account, action, txId) {
  ...
  const permitted = await accountRules.isAuthorized(account);
  console.log(`⏳ ...`);    
  console.log(`🔍 isAuthorized(${account}) = ${permitted}`);
  
  if (action === 'add') {
    if (!permitted) {
      process.exitCode = 2;
      console.error('❌ ERROR: Not admin after completion. Check multisig threshold.');
    } else {
    ...
    if (permitted) {
      process.exitCode = 2;
      console.error('❌ ERROR: Still admin after completion. Check multisig threshold.');
    } else {
...
(async () => {
  ...
  // Get Admin contract address from Ingress
  ...
  const accountRulesAddress = await ingress.getContractAddress(ADMIN_KEY);
  
  if (!accountRulesAddress || accountRulesAddress === ethers.constants.AddressZero) {
    throw new Error('❌ Admin address is not configured in Ingress');
  }
  
  console.log(`🔗 Using Admin at: ${accountRulesAddress}`);
  ...
  const shouldContinue = await checkPermissionStatus(accountRules1, NEW_ACCOUNT, ACTION);
  if (!shouldContinue) return;
  
  // Step 3: Submit transaction with signer1
  const tx = await submitTransaction(accountRules1, ACTION, NEW_ACCOUNT, signer1Address);
  await waitForConfirmation(tx);
  
  // Step 4: Check if transaction auto-executed (single-sig mode)
  const isPermittedNow = await accountRules1.isAuthorized(NEW_ACCOUNT);
  ...
  if (isPermittedNow === expectedPermission) {
    ...
    console.log(`🔍 isAuthorized(${NEW_ACCOUNT}) = ${isPermittedNow}`);
    console.log(`🎉 Done: ${ACTION === 'add' ? 'admin added' : 'admin removed'} successfully.`);
    return;
  }
