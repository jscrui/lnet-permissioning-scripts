...
const FUNCTION_SIGNATURES = {
  ADD_TARGET: ethers.utils.id('addTarget(address)').slice(0, 10),
  REMOVE_TARGET: ethers.utils.id('removeTarget(address)').slice(0, 10)
};
...
// ABI for Ingress contract - used to get the Rules and Admin contract addresses
...
// ABI for AccountRules - handles target permissioning and multisig
const ACCOUNT_RULES_ABI = [
  'function addTarget(address target) returns (bool)',
  'function removeTarget(address target) returns (bool)',
  'function destinationPermitted(address target) view returns (bool)',
...
async function findPendingTransactionTxId(accountRules, targetAddress, action) {
  ...
  const targetSigs = action === 'add' 
    ? [FUNCTION_SIGNATURES.ADD_TARGET]
    : [FUNCTION_SIGNATURES.REMOVE_TARGET];
...
async function checkPermissionStatus(accountRules, account, action) {
  const isPermitted = await accountRules.destinationPermitted(account);
  ...
  if (action === 'add') {
    ...
    console.log(`🗑️ Revoking target: ${account}`);
  }
...
async function submitTransaction(accountRules1, action, account, signer1Address) {
  ...
  if (action === 'add') {
    const gasLimit = await estimateGasWithFallback(
      (baseGas) => accountRules1.estimate.addTarget(account, { gasLimit: baseGas, gasPrice: 0 }),
      GAS_LIMIT_ADD_ACCOUNT,
      GAS_BUFFER
    );
    tx = await accountRules1.functions.addTarget(account, { gasLimit, gasPrice: 0 });
    console.log(`📤 ➕ addTarget(address) | sent by initiator (${signer1Address})`);
  } else {
    const gasLimit = await estimateGasWithFallback(
      (baseGas) => accountRules1.estimate.removeTarget(account, { gasLimit: baseGas, gasPrice: 0 }),
      GAS_LIMIT_REMOVE_ACCOUNT,
      GAS_BUFFER
    );
    tx = await accountRules1.functions.removeTarget(account, { gasLimit, gasPrice: 0 });
    console.log(`📤 ➖ removeTarget(address) | sent by initiator (${signer1Address})`);
  }
...
async function verifyFinalState(accountRules, account, action, txId) {
  ...
  const permitted = await accountRules.destinationPermitted(account);
  console.log(`⏳ ...`);    
  console.log(`🔍 destinationPermitted(${account}) = ${permitted}`);
  ...
  if (action === 'add') {
    ...
    if (permitted) {
      ...
      console.log('🎉 Success: TARGET REMOVED!');
    }
  }
}
...
(async () => {
  ...
  // Get Rules and Admin contract addresses from Ingress
  ...
  if (!accountRulesAddress || accountRulesAddress === ethers.constants.AddressZero) {
    throw new Error('❌ AccountRules address is not configured in Ingress');
  }
  ...
  const isPermittedNow = await accountRules1.destinationPermitted(NEW_ACCOUNT);
  ...
  if (isPermittedNow === expectedPermission) {
    ...
    console.log(`🔍 destinationPermitted(${NEW_ACCOUNT}) = ${isPermittedNow}`);
    console.log(`🎉 Done: ${ACTION === 'add' ? 'target added' : 'target removed'} successfully.`);
    return;
  }
